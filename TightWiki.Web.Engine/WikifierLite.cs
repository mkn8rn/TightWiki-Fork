...
namespace TightWiki.Web.Engine
{
    ...
    {
        /// <summary>
        /// Process wiki markup with minimal features (for comments, bios, etc.)
        /// </summary>
        /// <param name="unprocessedText">The text to process</param>
        /// <param name="config">Engine configuration with emoji data and base path</param>
        public static string Process(string? unprocessedText, IEngineConfiguration config)
        {
            ...
            for (int i = 0; i < 100; i++) //We don't want to process nested wiki forever.
            {
                ...
                TransformEmoji(content, matchStore, config.Emojis);
                TransformLinks(content, matchStore, config.BasePath);
        ...
        private static void TransformEmoji(WikiString pageContent, Dictionary<string, string> matchStore, IReadOnlyList<Emoji> emojis)
        {
            ...
            foreach (var match in matches)
            {
                ...
                var emoji = emojis.FirstOrDefault(o => o.Shortcut == key);

                if (emojis.Any(o => o.Shortcut == key))
                {
        ...
        /// </summary>
        /// <param name="pageContent"></param>
        private static void TransformLinks(WikiString pageContent, Dictionary<string, string> matchStore, string basePath)
        {
            ...
            foreach (var match in matches)
            {
                ...
                if (args.Count == 1)
                {
                    pageContent.Replace(match.Value, StoreMatch(matchStore, $"<a href=\"{basePath}/{args[0]}\">{args[0]}</a>"));
                }
                else if (args.Count > 1)
                {
                    pageContent.Replace(match.Value, StoreMatch(matchStore, $"<a href=\"{basePath}/{args[0]}\">{args[1]}</a>"));
                }
            }
        }
    }
