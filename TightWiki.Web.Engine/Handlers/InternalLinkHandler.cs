...
namespace TightWiki.Web.Engine.Handlers
{
    ...
    public class InternalLinkHandler : IInternalLinkHandler
    {
        ...
        {
            var page = state.DataProvider.GetPageByNavigation(pageNavigation.Canonical);
            ...
            if (page == null)
            {
                if (state.Session?.HoldsPermission(pageNavigation.Canonical, WikiPermission.Create) == true)
                {
                    if (image != null)
                    {
                            ...
                            || image.StartsWith("https://", StringComparison.InvariantCultureIgnoreCase))
                        {
                            ...
                            href = $"<a href=\"{state.Config.BasePath}/Page/Create?Name={pageName}\"><img src=\"{state.Config.BasePath}{image}?Scale={imageScale}\" /></a>";
                        }
                        else if (image.Contains('/'))
                        {
                            //The image is located on another page.
                            href = $"<a href=\"{state.Config.BasePath}/Page/Create?Name={pageName}\"><img src=\"{state.Config.BasePath}/Page/Image/{image}?Scale={imageScale}\" /></a>";
                        }
                        else
                        {
                            //The image is located on this page, but this page does not exist.
                            href = $"<a href=\"{state.Config.BasePath}/Page/Create?Name={pageName}\">{linkText}</a>";
                        }
                    ...
                    {
                        var href = $"<a href=\"{state.Config.BasePath}/Page/Create?Name={pageName}\">{linkText}</a>"
                ...
                else
                {
                    ...
                    if (image != null)
                    {
                        ...
                        {
                            //The image is external.
                            mockHref = $"<img src=\"{state.Config.BasePath}{image}?Scale={imageScale}\" />";
                        }
                        else if (image.Contains('/'))
                        {
                            //The image is located on another page.
                            mockHref = $"<img src=\"{state.Config.BasePath}/Page/Image/{image}?Scale={imageScale}\" />";
                        }
                        else
                        {
                            //The image is located on this page, but this page does not exist.
            ...
            else
            {
                ...
                if (image != null)
                {
                    ...
                    {
                        //The image is external.
                        href = $"<a href=\"{state.Config.BasePath}/{page.Navigation}\"><img src=\"{state.Config.BasePath}{image}\" /></a>";
                    }
                    else if (image.Contains('/'))
                    {
                        //The image is located on another page.
                        href = $"<a href=\"{state.Config.BasePath}/{page.Navigation}\"><img src=\"{state.Config.BasePath}/Page/Image/{image}?Scale={imageScale}\" /></a>";
                    }
                    else
                    {
                        //The image is located on this page.
                        href = $"<a href=\"{state.Config.BasePath}/{page.Navigation}\"><img src=\"{state.Config.BasePath}/Page/Image/{state.Page.Navigation}/{image}?Scale={imageScale}\" /></a>";
                    }
                }
                else
                {
                    //Just a plain ol' internal page link.
                    href = $"<a href=\"{state.Config.BasePath}/{page.Navigation}\">{linkText}</a>";
                }
