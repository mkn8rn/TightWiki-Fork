...
using TightWiki.Web.Engine.Function;
...
using static TightWiki.Web.Engine.Function.FunctionConstants;
...
namespace TightWiki.Web.Engine.Handlers
{
    ...
    public class StandardFunctionHandler : IStandardFunctionHandler
    {
        ...
        private static EnginePage? GetPageFromPathInfo(ITightEngineState state, string routeData)
        {
            routeData = NamespaceNavigation.CleanAndValidate(routeData);
            var page = state.DataProvider.GetPageByNavigation(routeData);
            return page;
        }
        ...
        public HandlerResult Handle(ITightEngineState state, FunctionCall function, string? scopeBody = null)
        {
            switch (function.Name.ToLowerInvariant())
            {
                ...
                case "profileglossary":
                    {
                        if (!state.Config.EnablePublicProfiles)
                        {
                        ...
                        var profiles = state.DataProvider.GetPublicProfilesPaged(pageNumber, pageSize, searchToken);
                        ...
                        if (profiles.Count > 0)
                        {
                            ...
                            foreach (var alpha in alphabet)
                            {
                                ...
                                {
                                    html.Append($"<li><a href=\"{state.Config.BasePath}/Profile/{profile.Navigation}/Public\">{profile.AccountName}</a>");
                                    html.Append("</li>");
                                }
                                html.Append("</ul>");
                            }
                ...
                case "attachments":
                    {
                        ...
                        var attachments = state.DataProvider.GetPageFilesPaged(navigation, pageNumber, pageSize, state.Revision);
                        var html = new StringBuilder();
                        ...
                        if (attachments.Count > 0)
                        {
                            ...
                            foreach (var file in attachments)
                            {
                                ...
                                {
                                    html.Append($"<li><a href=\"{state.Config.BasePath}/Page/Binary/{state.Page.Navigation}/{file.FileNavigation}/{state.Revision}\">{file.Name}</a>");
                                }
                                else
                                {
                                    html.Append($"<li><a href=\"{state.Config.BasePath}/Page/Binary/{state.Page.Navigation}/{file.FileNavigation}\">{file.Name} </a>");
                                }
                ...
                case "revisions":
                    {
                        ...
                        var pageSize = function.Parameters.Get<int>("pageSize");
                        var pageSelector = function.Parameters.Get<bool>("pageSelector");
                        var revisions = state.DataProvider.GetPageRevisionsInfoByNavigationPaged(navigation, pageNumber, null, null, pageSize);
                        var html = new StringBuilder();
                        ...
                        if (revisions.Count > 0)
                        {
                            ...
                            {
                                html.Append($"<li><a href=\"{state.Config.BasePath}/{item.Navigation}/{item.Revision}\">{item.Revision} by {item.ModifiedByUserName} on {state.Session.LocalizeDateTime(item.ModifiedDate)}</a>");

                                if (styleName == "full")
                                {
                                    var thisRev = state.DataProvider.GetPageByNavigation(state.Page.Navigation, item.Revision);
                                    var prevRev = state.DataProvider.GetPageByNavigation(state.Page.Navigation, item.Revision - 1);
                ...
                case "editlink": //(##EditLink(link text))
                    {
                        ...
                        return new HandlerResult($"<a href=\"{state.Config.BasePath}" + NamespaceNavigation.CleanAndValidate($"/{state.Page.Navigation}/Edit") + $"\">{linkText}</a>");
                    }
                ...
                case "inject": //(PageName)
                    {
                        ...
                        var page = GetPageFromPathInfo(state, navigation);
                        if (page != null)
                        {
                ...
                case "image": //##Image(Name, [optional:default=100]Scale, [optional:default=""]Alt-Text)
                    {
                        ...
                        {
                            if (state.DataProvider.GetFileAttachment(navigation, NamespaceNavigation.CleanAndValidate(imageName), state.Revision) == null)
                            {
                        ...
                        if (state.Revision != null && isPageForeignImage == false)
                        {
                            ...
                            string image = $"<a href=\"{state.Config.BasePath}{link}\" target=\"_blank\"><img src=\"{state.Config.BasePath}{link}?{string.Join('&', queryParams)}\" border=\"0\" alt=\"{alt}\" {imgClass} /></a>";
                            return new HandlerResult(image);
                        }
                        else
                        {
                ...
                case "file": //##file(Name | Alt-Text | [optional display file size] true/false)
                    {
                        ...
                        if (explicitNamespace == false && state.Page.Namespace != null)
                        {
                            if (state.DataProvider.GetFileAttachment(navigation, NamespaceNavigation.CleanAndValidate(fileName), state.Revision) == null)
                            {
                                //If the image does not exist, and no namespace was specified, but the page has a namespace - then default to the pages namespace.
                        ...
                        var attachment = state.DataProvider.GetFileAttachment(navigation, NamespaceNavigation.CleanAndValidate(fileName), state.Revision);
                        if (attachment != null)
                        {
                            ...
                            if (state.Revision != null && isPageForeignFile == false)
                            {
                                ...
                                string image = $"<a href=\"{state.Config.BasePath}{link}\">{alt}</a>";
                                return new HandlerResult(image);
                            }
                            else
                            {
                ...
                case "recentlymodified": //##RecentlyModified(TopCount)
                    {
                        ...
                        var pages = state.DataProvider.GetTopRecentlyModifiedPages(takeCount)
                        ...
                        if (pages.Count > 0)
                        {
                            ...
                            foreach (var page in pages)
                            {
                                if (showNamespace)
                                {
                                    html.Append($"<li><a href=\"{state.Config.BasePath}/{page.Navigation}\">{page.Name}</a>");
                                }
                                else
                                {
                                    html.Append($"<li><a href=\"{state.Config.BasePath}/{page.Navigation}\">{page.Title}</a>");
                                }
                ...
                case "namespaceglossary":
                    {
                        ...
                        string styleName = function.Parameters.Get<string>("styleName").ToLowerInvariant();
                        var topCount = function.Parameters.Get<int>("top");
                        var pages = state.DataProvider.GetPagesByNamespaces(namespaces).Take(topCount).OrderBy(o => o.Name).ToList();
                        var html = new StringBuilder();
                ...
                case "tagglossary":
                    {
                        ...
                        string styleName = function.Parameters.Get<string>("styleName").ToLowerInvariant();
                        var topCount = function.Parameters.Get<int>("top");
                        var pages = state.DataProvider.GetPagesByTags(tags).Take(topCount).OrderBy(o => o.Name).ToList();
                        var html = new StringBuilder();
                        var alphabet = pages.Select(p => p.Title.Substring(0, 1).ToUpperInvariant()).Distinct();
                        var showNamespace = function.Parameters.Get<bool>("showNamespace");
                ...
                case "textglossary":
                    {
                        ...
                        var pages = state.DataProvider.SearchPages(searchTokens).Take(topCount).OrderBy(o => o.Name).ToList();
                        var html = new StringBuilder();
                        var alphabet = pages.Select(p => p.Title.Substring(0, 1).ToUpperInvariant()).Distinct();
                        string styleName = function.Parameters.Get<string>("styleName").ToLowerInvariant();
                ...
                case "searchlist":
                    {
                        ...
                        var pages = state.DataProvider.SearchPagesPaged(searchTokens, pageNumber, pageSize, allowFuzzyMatching);
                        var html = new StringBuilder();
                ...
                case "similar": //##Similar()
                    {
                        ...
                        var pages = state.DataProvider.GetSimilarPagesPaged(state.Page.Id, similarity, pageNumber, pageSize);
                        ...
                        else if (styleName == "flat")
                        {
                            foreach (var page in pages)
                            {
                                ...
                                html.Append($"<a href=\"{state.Config.BasePath}/{page.Navigation}\">{page.Title}</a>");
                            }
                        }
                        else if (styleName == "full")
                        {
                            ...
                            foreach (var page in pages)
                            {
                                html.Append($"<li><a href=\"{state.Config.BasePath}/{page.Navigation}\">{page.Title}</a> - {page.Description}");
                            }
                            html.Append("</ul>");
                        }
                ...
                case "related": //##related
                    {
                        ...
                        var pages = state.DataProvider.GetRelatedPagesPaged(state.Page.Id, pageNumber, pageSize);
                ...
                case "sitename":
                    {
                        return new HandlerResult(state.Config.SiteName);
                    }
                ...
                case "systememojilist":
                    {
                        ...
                        {
                            // Filter emojis by category from GlobalConfiguration (already loaded by IEmojiService)
                            var emojis = state.Config.Emojis
                                .Where(e => e.Categories.Split(';', StringSplitOptions.RemoveEmptyEntries)
                            ...
                            foreach (var emoji in emojis)
                            {
                                ...
                                html.Append($"<td><img src=\"{state.Config.BasePath}/File/Emoji/{emoji.Name.ToLowerInvariant()}\" /></td>");
                ...
                case "systememojicategorylist":
                    {
                        // Get emoji categories grouped from GlobalConfiguration (already loaded by IEmojiService)
                        var categories = state.Config.Emojis
                            .SelectMany(e => e.Categories.Split(';', StringSplitOptions.RemoveEmptyEntries))
                            ...
                            .OrderByDescending(c => int.Parse(c.EmojiCount))
                            .ThenBy(c => c.Category);
                        ...
                        foreach (var category in categories)
                        {
                            ...
                            html.Append($"<tr>");
                            html.Append($"<td><a href=\"{state.Config.BasePath}/wiki_help::list_of_emojis_by_category?category={category.Category}\">{category.Category}</a></td>");
