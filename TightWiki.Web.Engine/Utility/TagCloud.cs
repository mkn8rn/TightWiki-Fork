...
namespace TightWiki.Web.Engine.Utility
{
    public static class TagCloud
    {
        public static string Build(string basePath, IEngineDataProvider dataProvider, string seedTag, int? maxCount)
        {
            var tags = dataProvider.GetAssociatedTags(seedTag).OrderByDescending(o => o.PageCount).ToList();
            ...
            {
                tagList.Add(new TagCloudItem(tag.Tag, tagIndex, "<font size=\"" + fontSize + $"\"><a href=\"{basePath}/Tag/Browse/" + NamespaceNavigation.CleanAndValidate(tag.Tag) + "\">" + tag.Tag + "</a></font>"));
