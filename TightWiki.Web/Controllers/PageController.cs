...
namespace TightWiki.Controllers
{
    ...
    {
        private readonly IEngineConfiguration _engineConfig = engineConfigProvider.GetEngineConfiguration();
        private readonly IEngineDataProvider _engineDataProvider = engineDataProvider;
        ...
        public IActionResult Display(string givenCanonical, int? pageRevision)
        {
            ...
            var page = pageService.GetPageRevisionByNavigation(navigation.Canonical, pageRevision);
            if (page != null)
            {
                ...
                if (GlobalConfiguration.PageCacheSeconds > 0)
                {
                    ...
                    {
                        var state = tightEngine.Transform(_engineConfig, _engineDataProvider, SessionState, page, pageRevision);
                ...
                {
                    var comments = pageService.GetPageCommentsPaged(navigation.Canonical, 1);
            ...
            else if (pageRevision != null)
            {
                ...
                var notExistsPage = pageService.GetPageRevisionByNavigation(notExistPageNavigation).EnsureNotNull();

                SessionState.SetPageId(null, pageRevision);

                var state = tightEngine.Transform(_engineConfig, _engineDataProvider, SessionState, notExistsPage);
        ...
        {
            var pages = pageService.AutoCompletePage(q);
        ...
        public ActionResult Search()
        {
            ...
            if (string.IsNullOrEmpty(searchString) == false)
            {
                ...
                {
                    Pages = pageService.PageSearchPaged(Utility.SplitToTokens(searchString), GetQueryValue("page", 1)),
        ...
        public ActionResult Comments(string givenCanonical)
        {
            ...
            if (string.IsNullOrEmpty(deleteAction) == false && SessionState.IsAuthenticated)
            {
                if (SessionState.HoldsPermission(givenCanonical, WikiPermission.Moderate))
                {
                    ...
                    pageService.DeletePageCommentById(pageInfo.Id, int.Parse(deleteAction));
                }
                else
                {
                    pageService.DeletePageCommentByUserAndId(pageInfo.Id, SessionState.Profile.EnsureNotNull().UserId, int.Parse(deleteAction));
                }
            }

            var model = new PageCommentsViewModel();

            var comments = pageService.GetPageCommentsPaged(pageNavigation, GetQueryValue("page", 1));
            foreach (var comment in comments)
            {
                model.Comments.Add(new PageComment
                {
        ...
        public ActionResult Comments(PageCommentsViewModel model, string givenCanonical)
        {
            ...
            pageService.InsertPageComment(pageInfo.Id, SessionState.Profile.EnsureNotNull().UserId, model.Comment);
        ...
        public ActionResult Refresh(string givenCanonical)
        {
            ...
            if (page != null)
            {
                ...
                TightWiki.Web.Engine.Helpers.RefreshPageMetadata(tightEngine, _engineConfig, _engineDataProvider, enginePage, SessionState);
            }
        ...
        public ActionResult Compare(string givenCanonical, int pageRevision)
        {
            ...
            var thisRev = pageService.GetPageRevisionByNavigation(pageNavigation, pageRevision);
            var prevRev = pageService.GetPageRevisionByNavigation(pageNavigation, pageRevision - 1);
        ...
        public ActionResult Revisions(string givenCanonical)
        {
            ...
            {
                Revisions = pageService.GetPageRevisionsInfoByNavigationPaged(pageNavigation, pageNumber, orderBy, orderByDirection)
            };
        ...
        public ActionResult Delete(string givenCanonical, PageDeleteViewModel model)
        {
            ...
            var instructions = pageService.GetPageProcessingInstructionsByPageId(page.EnsureNotNull().Id);
            ...
            {
                pageService.MovePageToDeletedById(page.Id, (SessionState.Profile?.UserId).EnsureNotNullOrEmpty());
        ...
        public ActionResult Delete(string givenCanonical)
        {
            ...
            var page = pageService.GetPageRevisionByNavigation(pageNavigation).EnsureNotNull();
        ...
        public ActionResult Revert(string givenCanonical, int pageRevision, PageRevertViewModel model)
        {
            ...
            {
                var page = pageService.GetPageRevisionByNavigation(pageNavigation, pageRevision).EnsureNotNull();
                var enginePage = MapToEnginePage(page);
                TightWiki.Web.Engine.Helpers.UpsertPage(tightEngine, _engineConfig, _engineDataProvider, enginePage, SessionState);
        ...
        public ActionResult Revert(string givenCanonical, int pageRevision)
        {
            ...
            var mostCurrentPage = pageService.GetPageRevisionByNavigation(pageNavigation).EnsureNotNull();
            ...
            var revisionPage = pageService.GetPageRevisionByNavigation(pageNavigation, pageRevision).EnsureNotNull();
        ...
        public ActionResult Edit(string givenCanonical)
        {
            ...
            else
            {
                ...
                string templateName = configurationService.GetConfigurationValue<string>(Constants.ConfigurationGroup.Customization, "New Page Template").EnsureNotNull();
        ...
        public ActionResult Edit(PageEditViewModel model)
        {
            ...
            model.FeatureTemplates = pageService.GetAllFeatureTemplates();
            ...
            if (model.Id == 0) //Saving a new page.
            {
                ...
                if (pageService.GetPageInfoByNavigation(page.Navigation) != null)
                {
                ...
                var enginePage = MapToEnginePage(page);
                page.Id = TightWiki.Web.Engine.Helpers.UpsertPage(tightEngine, _engineConfig, _engineDataProvider, enginePage, SessionState);
            ...
            else
            {
                ...
                var page = pageService.GetPageRevisionById(model.Id).EnsureNotNull();
                var instructions = pageService.GetPageProcessingInstructionsByPageId(page.Id);
                ...
                {
                    if (pageService.GetPageInfoByNavigation(model.Navigation) != null)
                    {
                        ModelState.AddModelError("Name", Localize("The page name you entered already exists."));
                        return View(model);
                    }
        ...
        {
            var template = pageService.GetPageRevisionById(id);
            if (template == null)
        ...
        public ActionResult Image(string givenPageNavigation, string givenFileNavigation, int? pageRevision = null)
        {
            ...
            var file = pageFileService.GetPageFileAttachmentByPageNavigationPageRevisionAndFileNavigation(pageNavigation.Canonical, fileNavigation.Canonical, pageRevision);
        ...
        public IActionResult Export(string givenCanonical)
        {
            ...
            var page = pageService.GetPageRevisionByNavigation(navigation.Canonical);
            if (page == null)
                return NotFound();
