...
{
    [Route("[controller]")]
    public class ProfileController : WikiControllerBase<ProfileController>
    {
        ...
        public ActionResult Avatar(string userAccountName)
        {
            ...
            {
                avatar = _usersService.GetProfileAvatarByNavigation(NamespaceNavigation.CleanAndValidate(userAccountName))
                    ?? new ProfileAvatar();
            }
            else
            {
        ...
        public ActionResult Public(string userAccountName)
        {
            ...
            var accountProfile = _usersService.GetBasicProfileByUserId(
                _usersService.GetUserAccountIdByNavigation(userAccountName) ?? Guid.Empty);
            ...
            model.RecentlyModified = _pageService.GetTopRecentlyModifiedPagesInfoByUserId(accountProfile.UserId, GlobalConfiguration.DefaultProfileRecentlyModifiedCount)
                .OrderByDescending(o => o.ModifiedDate).ThenBy(o => o.Name).ToList();

            foreach (var item in model.RecentlyModified)
            {
                var thisRev = _pageService.GetPageRevisionByNavigation(item.Navigation, item.Revision);
                var prevRev = _pageService.GetPageRevisionByNavigation(item.Navigation, item.Revision - 1);
        ...
        public ActionResult My()
        {
            ...
            var model = new AccountProfileViewModel()
            {
                AccountProfile = AccountProfileAccountViewModel.FromDataModel(
                    _usersService.GetAccountProfileByUserId(SessionState.Profile.EnsureNotNull().UserId)),
        ...
        public ActionResult My(AccountProfileViewModel model)
        {
            ...
            var profile = _usersService.GetAccountProfileByUserId(userId);
        ...
        public ActionResult DeleteAccount(ConfirmActionViewModel model)
        {
            ...
            var profile = _usersService.GetBasicProfileByUserId(SessionState.Profile.EnsureNotNull().UserId);
